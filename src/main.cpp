...
int main(int argc, char* argv[])
{
    ...
    Mat homography =
        match(image_1, image_2, dmatchs, keypoints_1, keypoints_2, change_model, right_matchs, matched_lines);
    ...
    // 把正确匹配点坐标写入文件中
    ofstream ofile;
    ofile.open("./image_save/position.txt");
    for (size_t i = 0; i < right_matchs.size(); ++i) {
        ofile << keypoints_1[right_matchs[i].queryIdx].pt << "   " << keypoints_2[right_matchs[i].trainIdx].pt << endl;
    }
