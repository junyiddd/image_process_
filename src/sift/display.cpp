...
void mosaic_pyramid(const vector<vector<Mat>>& pyramid, Mat& pyramid_image, int nOctaceLayers, string str)
{
    ...
    for (auto beg = pyramid.cbegin(); beg != pyramid.cend(); ++beg, ++i) {
        ...
        for (auto it = (*beg).cbegin(); it != (*beg).cend(); ++it) {
            ...
            Mat temp(pyramid_image,
                     Rect(accumulate_col - temp_size[i][1], accumulate_row - temp_size[i][0], it->cols, it->rows));
            Mat temp_it;
 ...
 */
void write_mosaic_pyramid(const vector<vector<Mat>>& gauss_pyr_1,
                          const vector<vector<Mat>>& dog_pyr_1,
                          const vector<vector<Mat>>& gauss_pyr_2,
                          const vector<vector<Mat>>& dog_pyr_2,
                          int nOctaveLayers)
{
    // 显示参考和待配准高斯金字塔图像
