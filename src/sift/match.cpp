...
static Mat LMS(const Mat& match1_xy, const Mat& match2_xy, string model, float& rmse)
{
    ...
    if (!(model == string("affine") || model == string("similarity") || model == string("perspective")))
        CV_Error(Error::StsBadArg, "LMS模块图像变换类型输入错误！");
    ...
    if (model == string("affine")) {
        ...
        change =
            (Mat_<float>(3, 3) << values(0), values(1), values(4), values(2), values(3), values(5), +0.0f, +0.0f, 1.0f);
    ...
    // 如果是相似变换
        ...
        change = (Mat_<float>(3, 3) << values(0), values(1), values(2), values(1) * (-1.0f), values(0), values(3), +0.f,
                  +0.f, 1.f);
 ...
 */
Mat ransac(const vector<Point2f>& points_1,
           ...
           float& rmse)
{
    if (points_1.size() != points_2.size())
        CV_Error(Error::StsBadArg, "ransac模块输入特征点数量不一致！");

    if (!(model == string("affine") || model == string("similarity") || model == string("perspective")))
        CV_Error(Error::StsBadArg, "ransac模块图像变换类型输入错误！");
    ...
    for (size_t i = 0; i < iterations; ++i) {
        ...
        while (1) {
            ...
            // 保证这n个点坐标不相同
            if (n == 2 && p[0] != p[1] && (p10[p[0]] != p10[p[1]] || p11[p[0]] != p11[p[1]]) &&
            ...
            if (n == 4 && p[0] != p[1] && p[0] != p[2] && p[0] != p[3] && p[1] != p[2] && p[1] != p[3] &&
                p[2] != p[3] && (p10[p[0]] != p10[p[1]] || p11[p[0]] != p11[p[1]]) &&
                (p10[p[0]] != p10[p[2]] || p11[p[0]] != p11[p[2]]) &&
    ...
    if (k < num_ransac)
        CV_Error(Error::StsBadArg, "ransac模块删除错误点对后剩下正确点对个数不足以计算出变换关系矩阵！");
...
/*image_1表示参考图像
 ...
 width表示棋盘网格大小
 */
void mosaic_map(const Mat& image_1,
                const Mat& image_2,
                Mat& chessboard_1,
                Mat& chessboard_2,
                Mat& mosaic_image,
                int width)
{
    if (image_1.size != image_2.size)
        CV_Error(Error::StsBadArg, "mosaic_map模块输入两幅图大小必须一致！");
    ...
    switch (channels) {
        ...
        case 3: // 3通道
            for (auto beg = pos.cbegin(); beg != pos.cend(); ++beg) {
                ...
                // 排序
                std::sort(pixs.begin(), pixs.end(), [pix_val](const Vec3b& a, const Vec3b& b) -> bool {
 ...
 */
void image_fusion(const Mat& image_1,
                  const Mat& image_2,
                  const Mat T,
                  Mat& fusion_image,
                  Mat& mosaic_image,
                  Mat& matched_image)
{
    if (!(image_1.depth() == CV_8U && image_2.depth() == CV_8U))
        CV_Error(Error::StsBadArg, "image_fusion模块仅支持uchar类型图像！");
    if (image_1.channels() == 4 || image_2.channels() == 4)
        CV_Error(Error::StsBadArg, "image_fusion模块仅仅支持单通道或者3通道图像");
    ...
    if (channel_1 == 3 && channel_2 == 3) {
        ...
        image_1_temp = image_1;
        cvtColor(image_2, image_2_temp, COLOR_BGR2GRAY);
    } else if (channel_1 == 3 && channel_2 == 1) {
        cvtColor(image_1, image_1_temp, COLOR_BGR2GRAY);
        image_2_temp = image_2;
            ...
            for (int i = 0; i < nRows; ++i) {
                ...
                for (int j = 0; j < nCols; ++j) {
                    ...
                    } else // 对于重合区域
                    {
                        ...
                        ptr[nj + len + 1] =
                            saturate_cast<uchar>(((float)ptr[nj + len + 1] + (float)ptr_1[nj + len + 1]) / 2);
                        ptr[nj + len + 2] =
                            saturate_cast<uchar>(((float)ptr[nj + len + 2] + (float)ptr_1[nj + len + 2]) / 2);
                    }
                }
            }
            break;
    ...
    mosaic_map(trans_1(Y_range, X_range), trans_2(Y_range, X_range), chessboard_1, chessboard_2, mosaic_image, 50);
...
/*image_1表示参考图像，
 ...
 该函数返回变换模型参数
 */
Mat match(const Mat& image_1,
          const Mat& image_2,
          const vector<vector<DMatch>>& dmatchs,
          vector<KeyPoint> keys_1,
          vector<KeyPoint> keys_2,
          string model,
          ...
          Mat& matched_line)
{
    ...
    if ((int)point_1.size() < min_pairs)
        CV_Error(Error::StsBadArg, "match模块距离比阶段匹配特征点个数不足！");
    ...
    // 绘制初始匹配点对连线图
    Mat initial_matched;
    drawMatches(image_1, keys_1, image_2, keys_2, init_matchs, initial_matched, Scalar(0, 255, 0), Scalar(255, 0, 0),
                vector<char>(), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    imwrite("./image_save/初始匹配点对.jpg", initial_matched);

    // 绘制正确匹配点对连线图
    drawMatches(image_1, keys_1, image_2, keys_2, right_matchs, matched_line, Scalar(0, 255, 0), Scalar(255, 0, 0),
                vector<char>(), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    imwrite("./image_save/正确匹配点对.jpg", matched_line);

    // 保存和显示检测到的特征点
    Mat keys_image_1, keys_image_2;
    drawKeypoints(image_1, keys_1, keys_image_1, Scalar(0, 255, 0), cv::DrawMatchesFlags::DEFAULT);
    drawKeypoints(image_2, keys_2, keys_image_2, Scalar(0, 255, 0), cv::DrawMatchesFlags::DEFAULT);
