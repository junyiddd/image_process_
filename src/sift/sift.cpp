...
// #include <string>
#include <opencv2/core.hpp> //cv基本数据结构
#include <opencv2/core/hal/hal.hpp>
#include <opencv2/features2d.hpp> //特征提取
#include <opencv2/highgui.hpp>    //图像界面
#include <opencv2/imgproc.hpp>    //基本图像处理函数
        ...
        for (int j = 0; j < nOctaveLayers + 3; ++j) // 对于组内的每一层
        {
            if (i == 0 && j == 0) // 第一组，第一层
                ...
                resize(gauss_pyramid[i - 1][3], gauss_pyramid[i][0],
                       Size(gauss_pyramid[i - 1][3].cols / 2, gauss_pyramid[i - 1][3].rows / 2), 0, 0, INTER_LINEAR);
            } else {
                // 高斯滤波窗口大小选择很重要，这里选择(4*sig_diff_1+1)-(6*sig_diff+1)之间
...
static float clac_orientation_hist(const Mat& image, Point pt, float scale, int n, float* hist)
{
    ...
    hal::exp(W, W, len);
    hal::fastAtan2(Y, X, Ori, len, true); // 角度范围0-360度
    ...
    for (int i = 0; i < n; ++i) {
        hist[i] = (temp_hist[i - 2] + temp_hist[i + 2]) * (1.f / 16.f) +
                  (temp_hist[i - 1] + temp_hist[i + 1]) * (4.f / 16.f) + temp_hist[i] * (6.f / 16.f);
    }
...
static bool adjust_local_extrema(const vector<vector<Mat>>& dog_pyr,
                                 KeyPoint& kpt,
                                 int octave,
                                 int& layer,
                                 int& row,
                                 ...
                                 float sigma)
{
    ...
    for (; i < MAX_INTERP_STEPS; ++i) // 最大迭代次数
    {
        ...
        float dxy = (img.at<float>(row + 1, col + 1) + img.at<float>(row - 1, col - 1) -
                    ...
                    (1.f / 4.f);
        float dxz = (next.at<float>(row, col + 1) + prev.at<float>(row, col - 1) - next.at<float>(row, col - 1) -
                     prev.at<float>(row, col + 1)) *
                    (1.f / 4.f);
        float dyz = (next.at<float>(row + 1, col) + prev.at<float>(row - 1, col) - next.at<float>(row - 1, col) -
                     prev.at<float>(row + 1, col)) *
                    (1.f / 4.f);
        ...
        if (abs(xc) > (float)(INT_MAX / 3) || abs(xr) > (float)(INT_MAX / 3) || abs(xi) > (float)(INT_MAX / 3))
            return false;
        ...
        if (layer < 1 || layer > nOctaveLayers || col < IMG_BORDER || col > img.cols - IMG_BORDER || row < IMG_BORDER ||
            ...
            for (int r = IMG_BORDER; r < num_row - IMG_BORDER; ++r) {
                ...
                for (int c = IMG_BORDER; c < num_col - IMG_BORDER; ++c) {
                    ...
                    if (abs(val) > threshold &&
                        ...
                        if (!adjust_local_extrema(dog_pyr, kpt, octave, layer, r1, c1, nOctaveLayers,
                        ...
                        for (int i = 0; i < n; ++i) {
                            ...
                            if (hist[i] > hist[left] && hist[i] > hist[right] && hist[i] >= mag_thr) {
                                float bin =
                                    i + 0.5f * (hist[left] - hist[right]) / (hist[left] + hist[right] - 2 * hist[i]);
                                if (bin < 0)
                                    bin = bin + n;
                                if (bin >= n)
                                    bin = bin - n;
...
static void
calc_sift_descriptor(const Mat& gauss_image, float main_angle, Point2f pt, float scale, int d, int n, float* descriptor)
{
    ...
    // 计算像素梯度幅度，梯度角度，和高斯权值
    len = k;
    hal::fastAtan2(Y, X, Angle, len, true); // 角度范围是0-360度
    hal::magnitude(X, Y, Mag, len);         // 幅度
    hal::exp(W, W, len);                    // 高斯权值
...
void MySift::calc_descriptors(const vector<vector<Mat>>& gauss_pyr, vector<KeyPoint>& keypoints, Mat& descriptors) const
{
...
/*image表示输入的图像
 ...
 keypoints表示检测到的特征点*/
void MySift::detect(const Mat& image,
