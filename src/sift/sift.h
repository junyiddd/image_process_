...
public:
  ...
  // 创建高斯金字塔
  void build_gaussian_pyramid(const Mat&           init_image,
                              vector<vector<Mat>>& gauss_pyramid,
                              int                  nOctaves) const;

  // 创建高斯差分金字塔
  void build_dog_pyramid(vector<vector<Mat>>&       dog_pyramid,
                         const vector<vector<Mat>>& gauss_pyramid) const;

  // DOG金字塔特征点检测
  void find_scale_space_extrema(const vector<vector<Mat>>& dog_pyr,
                                const vector<vector<Mat>>& gauss_pyr,
                                vector<KeyPoint>&          keypoints) const;

  // 计算特征点的描述子
  void calc_descriptors(const vector<vector<Mat>>& dog_pyr,
  ...
  // 特征点描述
  void comput_des(const vector<vector<Mat>>& gauss_pyr,
                  vector<KeyPoint>&          keypoints,
                  Mat&                       descriptors) const;
